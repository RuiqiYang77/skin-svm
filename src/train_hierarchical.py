# 训练示例：
# python src/train_hierarchical.py --config config/hierarchical_svm_v4_xlc.yaml --experiment_id hier_exp001
#
# 参数说明：
# --config: YAML 配置文件路径
# --experiment_id: 实验编号，所有输出会保存到 outputs/{experiment_id}/
# --features_module: 特征提取模块（默认 src.dataloader.features_xlc）
...
from src.model.hierarchical_svm import (
    load_model_bundle,
    save_model_bundle,
    train_hierarchical_svm,
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the hierarchical SVM lesion classifier."
    )
...
def _make_svm_shim(config):
    """
    features_xlc.py and split.py read config["svm"]["features"] /
    config["svm"]["split"] etc. This shim aliases "hierarchical_svm" → "svm"
    so those utilities work without modification.
    """
    shim = dict(config)
    shim["svm"] = config["hierarchical_svm"]
    return shim
...
def main():
    ...
    if args.reuse_features and features_path.exists():
        ...
        features_df = extract_feature_table(metadata, shim_config)
        features_df.to_csv(features_path, index=False)

    split_df = create_grouped_split(metadata, shim_config)
